...
function Item({ItemName,ItemPrice,uri,removeItemFromWishlist,item,addItemToCart,navigation}) {
  return (
      <View style={{marginTop:7,
                    ...
                    width:width,
                    backgroundColor:'#fff',}}>
        <TouchableOpacity onPress={()=>navigation.navigate('ItemView',{'item':item})}>
        ...
        <View style={{flexDirection:'row',marginTop:10,height:40}}>
          <TouchableOpacity style={styles.IconDelete} activeOpacity={0.5}
                              onPress={()=>removeItemFromWishlist(item)}>
                <Icon name='trash'
                      type='font-awesome'
                      color={'#FFF'}/>
          </TouchableOpacity>
          <TouchableOpacity style={styles.IconAddCart} activeOpacity={0.5} 
