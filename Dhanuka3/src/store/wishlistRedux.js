...
function counterWishlist (state = initialState ,action){
    ...
    switch(action.type){
        case 'ADD_WISHLIST_ITEM':
          const num = state.wishlist.filter(Items => Items.id == action.wishList.id).length;
          if(num==0){
        ...
        case 'REMOVE_WISHLIST_ITEM':
          const newState1 = {wishlist : state.wishlist.filter(item => item.id !== action.wishList.id)}    
            return newState1
    }
    return state;
}
