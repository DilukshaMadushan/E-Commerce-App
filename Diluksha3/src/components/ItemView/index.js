...
import {addRelatedItemList, addHomeItemList2} from '../../store/relatedProductsRedux';
...
class ItemView extends Component {
  ...
  postingReview(rating) {
    ...
    console.log("Ratinggggg ",this.state.item.average_rating, "hi ", this.state.item.rating_count,"Rating my ", rating);
    ...
    console.log( parseFloat(this.state.item.average_rating), this.state.item.rating_count )

    const review = this.state.review;
    const average_rating =
      (parseFloat(this.state.item.average_rating) * this.state.item.rating_count + rating) /
    ...
    if (itemInList.length!==0){
      ...
      home_item_List[homeItemIndex].rating_count = home_item_List[homeItemIndex].rating_count+1;
  ...
  ItemImageShow() {
    if (this.state.item.images.length == 0) {
      return (
        <Image style={styles.ItemImage} source={require('../../images/home/man22.jpg')}></Image>
      );
    ...
    return (
      <View style={styles.categoryScreen}>
        <ScrollView style={{padding: 10}} ref="_scrollView">
          ...
          {(this.state.item.regular_price===this.state.item.price || this.state.item.regular_price==="")?<View></View>:
          <Text style={styles.ItemDiscount}>{Math.round(((parseFloat(this.state.item.regular_price)-parseFloat(this.state.item.price))/parseFloat(this.state.item.regular_price))*100)}% OFF</Text>}
          <View style={{flexDirection:'row', alignItems:'center',justifyContent:'center'}}>
              {(this.state.item.regular_price===this.state.item.price || this.state.item.regular_price==="")?<View></View>:
              <Text style={styles.ItemPriceRegular}>Rs. {this.state.item.regular_price}</Text>}
          ...
          <View style={{flexDirection:'row', alignItems:'center',justifyContent:'center', marginTop:10, marginBottom:5}}>
            <TouchableOpacity
              activeOpacity={0.6}
              delayPressIn={0}
              onPress={()=>{
              ...
              style={{width:30, backgroundColor:'#f7f8f8', alignItems:'center',justifyContent:'center', borderTopLeftRadius:25, borderBottomLeftRadius:25, borderWidth:1, borderColor:'#dbdcdc'}}
              >
                <Text style={{paddingVertical:7}}>-</Text>
            </TouchableOpacity>

            <View style={{width:50, backgroundColor:'#f7f8f8', alignItems:'center',justifyContent:'center',borderTopWidth:1, borderBottomWidth:1, borderColor:'#dbdcdc'}}>
              <Text style={{paddingVertical:7}}>{this.state.count.toString()}</Text>
            </View>
            ...
            <TouchableOpacity
              ...
              style={{width:30, backgroundColor:'#f7f8f8', alignItems:'center',justifyContent:'center', borderTopRightRadius:25, borderBottomRightRadius:25, borderWidth:1, borderColor:'#dbdcdc'}}
              >
                <Text style={{paddingVertical:7}}>+</Text>
            </TouchableOpacity>
          </View>
        ...
        <View style={{flexDirection: 'row', height: 45}}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              this.props.navigation.navigate('Delivery', {itemData:{isBuyItem:true,buyItem:[BuyingItem]}});
