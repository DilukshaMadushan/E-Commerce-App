...
class PaymentInputs extends Component {
  state = Object.assign(this.props.navigation.getParam('state'), {
    ShippingCost: 0,
    finalCost: this.props.TotalPrice,
  });
  ...
  componentWillMount() {
    ...
    if (this.state.itemData.isBuyItem) {
      finalCost =
        Number(this.state.itemData.buyItem[0].price) *
          Number(this.state.itemData.buyItem[0].count) +
        Number(this.state.deliveryCost);
    } else {
      finalCost =
    ...
    PostAPI.paymentInputsApi(
      JSON.stringify({
        ...
        shipping: {
          ...
          address_1: this.state.customerDetails.billing.address_1,
          address_2:
            this.state.customerDetails.billing.address_2 +
            ', ' +
            this.state.area,
          city: this.state.customerDetails.billing.city,
          state: this.state.customerDetails.billing.State,
  ...
  payFromPayhere = async () => {
    const response = await Toast.payOnce(
